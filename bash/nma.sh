...
APIkey="e7b0be8e783a874d652c501c5e7f976c21c7a16e8d6e80ef" ## API Key must me set here, go to http://www.notifymyandroid.com/ to get one
...
function pre_check {
## who doesn't check for required input before all this other crap? Oh, wait, the guys that wrote this...
if [ -z "${command}" -o -z "${application}" -o -z "${event}" -o -z "${description}" -o -z "${priority}" ]; then
	errormessage="You didn't pass any arguments, exiting."
	error_exit
fi
